...
TMap<FColor, FColoredMaterialRenderProxy*> UFGLineBatchComponent::GlobalColorMap;

FColoredMaterialRenderProxy* UFGLineBatchComponent::GetOrCreateColoredMaterial(const FColor& InColor)
{
	if (!GlobalColorMap.Contains(InColor))
	{
		FColoredMaterialRenderProxy* MaterialRenderProxy = new(FMemStack::Get()) FColoredMaterialRenderProxy(GEngine->DebugMeshMaterial->GetRenderProxy(), InColor);
		GlobalColorMap.Add(InColor, MaterialRenderProxy);
		return MaterialRenderProxy;
	}

	return GlobalColorMap[InColor];
}

FFGLineBatcherSceneProxy::FFGLineBatcherSceneProxy(const UFGLineBatchComponent* InComponent) :
...
SIZE_T FFGLineBatcherSceneProxy::GetTypeHash() const
{
	static size_t UniquePointer;
	return reinterpret_cast<size_t>(&UniquePointer);
}

void FFGLineBatcherSceneProxy::GetDynamicMeshElements(const TArray<const FSceneView*>& Views, const FSceneViewFamily& ViewFamily, uint32 VisibilityMap, FMeshElementCollector& Collector) const
{
	...
	for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
	{
		if (VisibilityMap & (1 << ViewIndex))
		{
			...
			for (int32 i = 0; i < Meshes.Num(); i++)
			{
				...
				FFGBatchedMesh const& M = Meshes[i];
				...
				FMaterialRenderProxy* MaterialRenderProxy = UFGLineBatchComponent::GetOrCreateColoredMaterial(M.Color);
...
*/
FPrimitiveViewRelevance FFGLineBatcherSceneProxy::GetViewRelevance(const FSceneView* View) const
{
...
uint32 FFGLineBatcherSceneProxy::GetMemoryFootprint( void ) const
{
	return( sizeof( *this ) + GetAllocatedSize() );
}

uint32 FFGLineBatcherSceneProxy::GetAllocatedSize( void ) const 
{ 
	return( FPrimitiveSceneProxy::GetAllocatedSize() + Lines.GetAllocatedSize() + Points.GetAllocatedSize() + Meshes.GetAllocatedSize() ); 
}


UFGLineBatchComponent::UFGLineBatchComponent(const FObjectInitializer& ObjectInitializer)
...
void UFGLineBatchComponent::DrawLine(const FVector& Start, const FVector& End, const FLinearColor& Color, uint8 DepthPriority, const float Thickness, const float LifeTime)
{
	BatchedLines.Add(FFGBatchedLine(Start, End, Color, LifeTime, Thickness, DepthPriority));

	// LineBatcher and PersistentLineBatcher components will be updated at the end of UWorld::Tick
	MarkRenderStateDirty();
}

void UFGLineBatchComponent::DrawLines(const TArray<FFGBatchedLine>& InLines)
{
...
void UFGLineBatchComponent::DrawPoint(
...
{
	new(BatchedPoints) FFGBatchedPoint(Position, Color, PointSize, LifeTime, DepthPriority);
	// LineBatcher and PersistentLineBatcher components will be updated at the end of UWorld::Tick
	MarkRenderStateDirty();
}

void UFGLineBatchComponent::DrawBox(const FBox& Box, const FMatrix& TM, const FColor& Color, uint8 InDepthPriorityGroup)
{
	FVector	B[2], P, Q;
	int32 ai, aj;
	B[0] = Box.Min;
	...
	for( ai=0; ai<2; ai++ ) for( aj=0; aj<2; aj++ )
	{
		...
		P.Z=B[0].Z; Q.Z=B[1].Z;
		BatchedLines.Add(FFGBatchedLine(TM.TransformPosition(P), TM.TransformPosition(Q), Color, DefaultLifeTime, 0.0f, InDepthPriorityGroup));
...
void UFGLineBatchComponent::DrawSolidBox(const FBox& Box, const FTransform& Xform, const FColor& Color, uint8 DepthPriority, float LifeTime)
{
	int32 const NewMeshIdx = BatchedMeshes.Add(FFGBatchedMesh());
	FFGBatchedMesh& BM = BatchedMeshes[NewMeshIdx];
	UFGLineBatchComponent::GetOrCreateColoredMaterial(Color);
	BM.Color = Color;
...
void UFGLineBatchComponent::DrawMesh(TArray<FVector> const& Verts, TArray<int32> const& Indices, FColor const& Color, uint8 DepthPriority, float LifeTime)
{
...
void UFGLineBatchComponent::DrawDirectionalArrow(const FMatrix& ArrowToWorld,FColor InColor,float Length,float ArrowSize,uint8 DepthPriority)
{
	const FVector Tip = ArrowToWorld.TransformPosition(FVector(Length,0,0));
	BatchedLines.Add(FFGBatchedLine(Tip,ArrowToWorld.TransformPosition(FVector::ZeroVector),InColor,DefaultLifeTime,0.0f,DepthPriority));
	BatchedLines.Add(FFGBatchedLine(Tip,ArrowToWorld.TransformPosition(FVector(Length-ArrowSize,+ArrowSize,+ArrowSize)),InColor,DefaultLifeTime,0.0f,DepthPriority));
	BatchedLines.Add(FFGBatchedLine(Tip,ArrowToWorld.TransformPosition(FVector(Length-ArrowSize,+ArrowSize,-ArrowSize)),InColor,DefaultLifeTime,0.0f,DepthPriority));
	BatchedLines.Add(FFGBatchedLine(Tip,ArrowToWorld.TransformPosition(FVector(Length-ArrowSize,-ArrowSize,+ArrowSize)),InColor,DefaultLifeTime,0.0f,DepthPriority));
	BatchedLines.Add(FFGBatchedLine(Tip,ArrowToWorld.TransformPosition(FVector(Length-ArrowSize,-ArrowSize,-ArrowSize)),InColor,DefaultLifeTime,0.0f,DepthPriority));
	MarkRenderStateDirty();
...
/** Draw a circle */
void UFGLineBatchComponent::DrawCircle(const FVector& Base,const FVector& X,const FVector& Y,FColor Color,float Radius,int32 NumSides,uint8 DepthPriority)
{
	...
	for(int32 SideIndex = 0;SideIndex < NumSides;SideIndex++)
	{
		...
		BatchedLines.Add(FFGBatchedLine(LastVertex,Vertex,Color,DefaultLifeTime,0.0f,DepthPriority));
		LastVertex = Vertex;
	}
...
void UFGLineBatchComponent::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction *ThisTickFunction)
{
	bool bDirty = false;
	...
	{
		FFGBatchedLine& Line = BatchedLines[LineIndex];
	...
	{
		FFGBatchedPoint& Pt = BatchedPoints[PtIndex];
	...
	{
		FFGBatchedMesh& Mesh = BatchedMeshes[MeshIndex];
...
void UFGLineBatchComponent::ApplyWorldOffset(const FVector& InOffset, bool bWorldShift)
{
	Super::ApplyWorldOffset(InOffset, bWorldShift);

	bool bDirty = false;
	for (FFGBatchedLine& Line : BatchedLines)
	{
		Line.Start += InOffset;
		Line.End += InOffset;
		bDirty = true;
	...
	for (FFGBatchedPoint& Point : BatchedPoints)
	{
		Point.Position += InOffset;
		bDirty = true;
	}

	for (FFGBatchedMesh& Mesh : BatchedMeshes)
	{
...
*/
FPrimitiveSceneProxy* UFGLineBatchComponent::CreateSceneProxy()
{
	return new FFGLineBatcherSceneProxy(this);
}

FBoxSphereBounds UFGLineBatchComponent::CalcBounds( const FTransform& LocalToWorld ) const 
{
	...
	FBox BBox(ForceInit);
	for (const FFGBatchedLine& Line : BatchedLines)
	{
		BBox += Line.Start;
		BBox += Line.End;
	}

	for (const FFGBatchedPoint& Point : BatchedPoints)
	{
		BBox += Point.Position;
	}

	for (const FFGBatchedMesh& Mesh : BatchedMeshes)
	{
...
void UFGLineBatchComponent::Flush()
{
